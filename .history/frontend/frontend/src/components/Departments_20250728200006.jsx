    ...
    <section className=" py-12 bg-white max-w-6xl mx-auto px-4">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <ul className='flex md:flex-col space-x-4 md:space-x-0 border-b md:border-b-0 md:border-r  border-gray-300'>
            ...
            {departments.map((dep)=>(
                <li key={dep._id}>
                    <button onClick={()=> handleTabClick(dep._id)} className={`w-40 mr-4 block mt-3 px-4 py-2 rounded-t md:rounded-tr-none  md:rounded-1 ${activeTab===dep._id ? "bg-white text-white":"bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>
                ...
                {departments?.map((dep)=>(
                    dep?._id === activeTab ? (
                        <div key={dep._id} className="flex flex-col md:flex-row items-center gap-6">
                            ...
                            <div className=''>
                                <h3 className='font-bold text-[#008e9b] text-2xl '>{dep?.name}</h3>
                                <p>{dep?.description}</p>
                            </div>
