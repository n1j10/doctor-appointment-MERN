  ...
  useEffect(() => {
    const fetchDoctor = async () => {
      try {
        ...
        fetchRelatedDoctors(data.specialty, data._id);
      } catch (err) {
        console.error(err);
    ...
    const fetchRelatedDoctors = async (specialty, currentId) => {
      try {
        const res = await fetch(`http://localhost:5000/doctors/bySpecialty/${specialty}`);
        const data = await res.json();
        if (!res.ok) throw new Error(data.message || "Failed to fetch related doctors");
        setRelatedDoctors(data.filter((doc) => doc._id !== currentId));
      } catch (err) {
        console.error(err);
      }
    };
