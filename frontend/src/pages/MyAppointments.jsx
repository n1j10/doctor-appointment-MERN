...
function MyAppointments() {
  ...
  const cancelAppointment = async(id) => {
    try {
      const token = localStorage.getItem("token")
      const res = await fetch(`${API_URL}/appointments/deleteAppointment/${id}`,{
        method:"POST",
         headers: { 
        ...
        const data = await res.json();
         ...
         setAppointments( prevAppointment=> {
          const updatedAppointments  = prevAppointment .filter((a)=> a._id !== id)
        ...
        <div className='space-y-6 max-w-3xl mx-auto'>
         ...
         <p className="text-center text-gray-500">No appointments found</p>
              ...
              appointments?.map((app)=> (
                <div key={app?._id} className="flex items-center justify-between bg-white shadow p-4 rounded-lg">
                  ...
                  <div className='flex  items-center gap-4'>
                    ...
                    <img className='w-20 h-20 rounded-full object-cover border' src={`${API_URL}/uploads/${app?.doctor?.image}`}/>
              ...
              <p>{new Date(app?.date).toLocaleDateString()}</p>
                  ...
                  <button className='text-white' onClick={()=> {if(window.confirm("Are you sure you want to cancel this appointment?")){
